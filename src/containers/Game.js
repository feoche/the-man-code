...
export default class Game extends Component {
    ...
    getVideoDuration(event) {
        setInterval(() => {
            this.setState({videoDuration: Math.floor(event.target.getDuration() - event.target.getCurrentTime())});
        }, 1000);
    }
            ...
            <div className="game-main">
                <div className="visual">
                    <YouTube
                        ...
                        className="video"
                        opts={opts}
                        onPlay={this.getVideoDuration.bind(this)}
                ...
                <div className="text">
                    {this.state.showSolution || this.state.videoDuration === 0 || this.state.videoDuration > 30 ? '' : <div className="progress">{this.state.videoDuration}</div>}
                    {this.state.nextQuestion ? '' : <div className="question">{currentQuestion.title}</div>}
                    <div className="answers">
                        {this.state.nextQuestion ?
                            <div className="answers"><span><a onClick={this.nextQuestion.bind(this)}
